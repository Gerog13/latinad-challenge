 ...
 */
export const workingHoursToJsonString = (
 ...
 */
export const jsonStringToWorkingHours = (
...
export const removeTimeRangeFromDay = (
  ...
  const updatedRanges = dayRanges.filter(
    (_: TimeRange, index: number) => index !== rangeIndex
  );
 ...
 */
export const formatWorkingHoursForDisplay = (
  workingHours: WorkingHours
): string[] => {
  ...
  WEEK_DAYS.forEach((day) => {
    ...
    if (ranges && ranges.length > 0) {
      const rangesText = ranges
        .map((range: TimeRange) => `${range.start} - ${range.end}`)
        .join(", ");
